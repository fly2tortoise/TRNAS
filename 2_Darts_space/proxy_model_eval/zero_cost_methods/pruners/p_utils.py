...
EPS = 1E-30
def adj_weights(model, inputs, targets, w=0.1, case=0, loss_maximize=False, ):
    # loss_maximize
    proxy_net = copy.deepcopy(model)
    proxy_net.train()
    proxy_optim = torch.optim.SGD(proxy_net.parameters(), lr=0.001) # 代理模型优化器
    proxy_net.double()

    if targets is None:
        loss = torch.sum(proxy_net(inputs.double()))
    else:
        loss = F.cross_entropy(proxy_net(inputs), targets)  # this

    if loss_maximize:  # True : 乘以 -1 的目的是让网络的权重更新朝着增加损失的方向变化，从而使得生成的对抗样本能够最大化误差。
        loss = -1*loss
    ...
    # 创建一个有序字典 diff_dict 来存储原始模型和代理模型的权重差异
    ...
    for (old_k, old_w), (new_k, new_w) in zip(model_state_dict.items(), proxy_state_dict.items()):
        if len(old_w.size()) <= 1:
            continue
        if 'weight' in old_k:
            diff_w = new_w - old_w  # 计算权重之间的差异
            # 计算每个权重的变化方向并按比例缩放，使得权重变化与原始权重的规模保持一致
            diff_dict[old_k] = old_w.norm() / (diff_w.norm() + EPS) * diff_w

    # diff_dict[name] 是代理模型和原始模型权重之间的差异
    names_in_diff = diff_dict.keys()
    with torch.no_grad():
        for name, param in model.named_parameters():
            if name in names_in_diff:
                param.add_(w*diff_dict[name]) # 对原始模型的权重进行调整。
...
def get_layer_metric_array_adv_feats(net, advnet, feats, adv_feats, metric, mode, search_space): 
    ...
    for layer, layer_adv in zip(net.modules(), advnet.modules()):
        ...
        if isinstance(layer, layer_type):  # 检查类型
            layer_cnt+=1

        # print(mode) 201 都是 param
        if mode=='channel' and hasattr(layer,'dont_ch_prune'):
            continue
        ...
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
            ...
            # print(_layer_cnt) # feats特征只有五层，但实际上metric计算量188次，这是否会构成不准确的问题？
