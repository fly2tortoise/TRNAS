...
defined proxy_model_eval space (7 vertices, 9 edges, 3 allowed ops). The first and last
...
the proxy_model_eval space, an OutOfDomainError will be raised, otherwise the data is
returned.
...
from nasbench_space.lib import config
from nasbench_space.lib import evaluate
from nasbench_space.lib import model_metrics_pb2
from nasbench_space.lib import model_spec as _model_spec
import numpy as np
...
class OutOfDomainError(Exception):
  """Indicates that the requested graph is outside of the proxy_model_eval domain."""
...
class NASBench(object):
  ...
  def query(self, model_spec, epochs=108, stop_halfway=False):
    ...
    Raises:
      OutOfDomainError: if model_spec or num_epochs is outside the proxy_model_eval space.
    """
