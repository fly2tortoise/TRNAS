...
def get_spec_from_arch_str(arch_str):
    ...
    spec = [OPNAME_TO_INDEX[op] for node in nodes for op in node]
    return spec
...
class NAS201Model(nn.Module):
    ...
    def forward(self, x, return_feats_only=False, return_activated_feats=False):
        activated_feats = []
        x = self.stem(x)
        activated_feats.append(x) # torch.Size([8, 16, 32, 32])

        x = self.stack_cell1(x)
        x = self.reduction1(x)
        activated_feats.append(x) # torch.Size([8, 32, 16, 16])

        x = self.stack_cell2(x)
        x = self.reduction2(x)
        activated_feats.append(x) # torch.Size([8, 64, 8, 8])

        x = self.stack_cell3(x)
        activated_feats.append(x) # torch.Size([8, 64, 8, 8])

        if return_feats_only:
            avg_feat = self.top(x, return_feats_only)
            activated_feats.append(avg_feat)
            return activated_feats
        
        if return_activated_feats:
            avg_feat, logit = self.top(x, return_activated_feats=return_activated_feats)
            # print(avg_feat.size(), logit.size()) # torch.Size([8, 64]) torch.Size([8, 10])
            activated_feats.append(avg_feat)       # torch.Size([8, 64])

            return logit, activated_feats
        else:
            del activated_feats
            x = self.top(x)
            return x
    ...
    def get_prunable_copy(self, bn=False):
        ...
        model_new.train() # strict=False 允许模型加载过程中存在参数不完全匹配的情况（即如果新模型和旧模型的结构不完全一致，也不会引发错误）
