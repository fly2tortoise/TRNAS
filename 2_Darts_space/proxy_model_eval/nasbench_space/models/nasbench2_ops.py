...
class top(nn.Module):
    def __init__(self, in_dims, num_classes, use_bn=True):
        ...
        self.global_pooling = nn.AdaptiveAvgPool2d(1)  # 自适应池化层，将输出长宽压缩到 1
        self.classifier  = nn.Linear(in_dims, num_classes)

    def forward(self, x, return_feats_only=False, return_activated_feats=False):
        x = self.lastact(x)            # torch.Size([8, 64, 8, 8])
        # print(x.size())
        x = self.global_pooling(x)     # torch.Size([8, 64, 1, 1])  每个通道里的8x8池化成
        # print(x.size())
        x = x.view(x.size(0), -1)      # torch.Size([8, 64])
        # print(x.size())

        if return_feats_only:
            return x
        
        logits = self.classifier(x)    # torch.Size([8, 10])
