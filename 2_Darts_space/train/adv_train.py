...
parser.add_argument('--data', type=str, default='../../0_Data', help='location of the data corpus')
...
parser.add_argument('--img_size', type=int, default=32, help='random seed')
parser.add_argument('--save_epoch', type=int, default=120, help='random seed')
args = parser.parse_args()        # img_size

args.save = 'C10-{}-{}-{}'.format(args.save, time.strftime("%m%d-%H%M%S"), args.arch)
...
def train(train_queue, model, criterion, optimizer, epoch):
  ...
  # for step, (input, target) in tqdm(enumerate(train_queue), disable=not is_tty): # tqdm(range(args.epochs), desc='Epoch', disable=not is_tty)
  for step, (input, target) in enumerate(train_queue):
    ...
    if args.adv_loss == 'pgd':
      loss = madry_loss( model, input, target, optimizer, step_size = args.step_size, epsilon = args.epsilon, perturb_steps = args.num_steps)
    elif args.adv_loss == 'trades':
      loss = trades_loss(model, input, target, optimizer, step_size=args.step_size, epsilon=args.epsilon, perturb_steps=args.num_steps, beta=args.beta, distance='l_inf')
...
def infer(valid_queue, model, criterion):
  ...
  with torch.no_grad():
    is_tty = sys.stdout.isatty()
    # for step, (input, target) in tqdm(enumerate(valid_queue), disable=not is_tty):
...
def main():
  ...
  FLOPs, Param = get_model_infos(model, [64, 3, args.img_size, args.img_size])
  logging.info("FLOPs size = %fMB", FLOPs)
  ...
  is_tty = sys.stdout.isatty()
  for epoch in tqdm(range(args.epochs), desc='Epoch', disable=not is_tty):
  # for epoch in tqdm(range(args.epochs), desc='Epoch'):
    ...
    if epoch >= 99 and epoch <= args.save_epoch:
