...
parser.add_argument('--test_batch_size', type=int, default=50, metavar='N',help='input batch size for testing (default: 200)')
...
parser.add_argument('--num-steps', type=int, default=1, help='perturb number of steps')                 # PGD 20 100
parser.add_argument('--step-size', default=8/255,help='step-size, 8 / 255 for FGSM, and 2 / 255 for PGD')
parser.add_argument('--random',default=False, help='True for PGD and False for FGSM')
parser.add_argument('--white-box-attack', default=True, help='whether perform white-box attack')
...
parser.add_argument('--max_epoch', type=int, default=120, help='Stop in FGSM test')
...
def main():
    ...
    cleans = []
    fgsms = []
    best_fgsm = float('-inf')  # 跟踪当前最优 FGSM
    ...
    for epoch, file_path in checkpoints:
        ...
        model.cuda()
        # 评估性能
        clean_acc, fgsm_acc = eval_adv_test_whitebox(model, device, test_loader)
        # 打印结果
        print(f"Epoch {epoch}: Clean = {clean_acc:.4f}, FGSM = {fgsm_acc:.4f}")

        # 更新最优 FGSM 和 Clean
        if fgsm_acc > best_fgsm:
            best_fgsm = fgsm_acc
        ...
        fgsms.append(fgsm_acc)

    # 使用 pandas 创建转置 DataFrame 并保存为 CSV
    data = {'Epoch': epochs, 'Clean': cleans, 'FGSM': fgsms}
    df = pd.DataFrame(data)
    df_transposed = df.set_index('Epoch').T
    # 添加 Best 列（最后一列）
    df_transposed['Best'] = [best_clean, best_fgsm]
    # 获取当前时间和架构信息
    current_time = datetime.now().strftime('%Y-%m-%d_%H-%M')
    # 构造输出文件名，包含时间和架构
    output_file = f'0.fgsm_{current_time}_{args.arch}.xlsx'
    # 保存到 Excel
